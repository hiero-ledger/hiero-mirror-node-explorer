...
describe("AccountDetails.vue", () => {
    ...
    it("AccountDetails_Operations should display recent transactions", async () => {
        ...
        expect(select.text()).toBe("TYPES: ALLADD LIVE HASHAPPROVE ALLOWANCEATOMIC BATCHCANCEL AIRDROP" +
            "CLAIM AIRDROPCONTRACT CALLCONTRACT CREATECONTRACT DELETECONTRACT UPDATECREATE ACCOUNTCREATE TOPIC" +
            "CRYPTO TRANSFERDELETE ACCOUNTDELETE ALLOWANCEDELETE LIVE HASHDELETE TOPICETHEREUM TRANSACTIONFILE APPEND" +
            "FILE CREATEFILE DELETEFILE UPDATEFREEZEHOOK STORENODE CREATENODE DELETENODE STAKE UPDATENODE UPDATE" +
            "PSEUDORANDOM NUMBER GENERATEREGISTERED NODE CREATEREGISTERED NODE DELETEREGISTERED NODE UPDATE" +
            "SCHEDULE CREATESCHEDULE DELETESCHEDULE SIGNSUBMIT MESSAGESYSTEM DELETESYSTEM UNDELETETOKEN AIRDROP" +
