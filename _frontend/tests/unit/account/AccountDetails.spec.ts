...
describe("AccountDetails.vue", () => {
    ...
    it("AccountDetails_Operations should display recent transactions", async () => {
        ...
        expect(select.text()).toBe("TYPES: ALLAPPROVE ALLOWANCEATOMIC BATCHCANCEL AIRDROPCLAIM AIRDROP" +
            "CONTRACT CALLCONTRACT CREATECONTRACT DELETECONTRACT UPDATECREATE ACCOUNTCREATE TOPICCRS PUBLICATION" +
            "CRYPTO TRANSFERDELETE ACCOUNTDELETE ALLOWANCEDELETE TOPICETHEREUM TRANSACTIONFILE APPENDFILE CREATE" +
            "FILE DELETEFILE UPDATEFREEZEHINTS KEY PUBLICATIONHINTS PARTIAL SIGNATUREHINTS PREPROCESSING VOTE" +
            "HISTORY PROOF KEY PUBLICATIONHISTORY PROOF SIGNATUREHISTORY PROOF VOTEHOOK STORELEDGER ID PUBLICATION" +
            "MIGRATION ROOT HASH VOTENODE CREATENODE DELETENODE STAKE UPDATENODE UPDATEPSEUDORANDOM NUMBER GENERATE" +
            "REGISTERED NODE CREATEREGISTERED NODE DELETEREGISTERED NODE UPDATESCHEDULE CREATESCHEDULE DELETE" +
            "SCHEDULE SIGNSTATE SIGNATURE TRANSACTIONSUBMIT MESSAGESYSTEM DELETESYSTEM UNDELETETOKEN AIRDROP" +
            "TOKEN ASSOCIATETOKEN BURNTOKEN CREATETOKEN DELETETOKEN DISSOCIATETOKEN FEE SCHEDULE UPDATETOKEN FREEZE" +
            "TOKEN KYC GRANTTOKEN KYC REVOKETOKEN MINTTOKEN PAUSETOKEN REJECTTOKEN UNFREEZETOKEN UNPAUSETOKEN UPDATE" +
