...
export function makeTokenName(token: TokenInfo | null, maxLength: number = TOKEN_NAME_DISPLAY_LENGTH): string {
    return truncateWithEllipsis(token?.name ?? '?', maxLength)
}

export function makeTokenSymbol(token: TokenInfo | null, maxLength: number = TOKEN_SYMBOL_DISPLAY_LENGTH): string {
